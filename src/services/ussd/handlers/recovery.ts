import { supabase, getBalance } from "../db";
import { t, isValidNationalId, isValidPin, hashPin } from "../utils";
import { RESTOCK_PHONE } from "../config";
...
async function recoverAccount(
  ...
  const userData = {
    ...
    name:                   oldUser.name,
    role:                   oldUser.role,
    merchant_code:          oldUser.role === 'merchant' ? (oldUser.merchant_code ?? null) : null,
    franchise_tier:         oldUser.role === 'merchant' ? (oldUser.franchise_tier ?? 'BASIC') : null,
    location:               oldUser.role === 'merchant' ? (oldUser.location ?? null) : null,
    acceptance_percent:     oldUser.role === 'merchant' ? (oldUser.acceptance_percent ?? null) : null,
    language:               lang,
  ...
  await supabase.from("users_uuid").insert(userData);
