...
export async function merchantFinalise(txn: any): Promise<boolean> {
  ...
  try {
    ...
    // The DB trigger handle_transaction_completion will handle ledger entries 
    // and is_first_purchase_used update automatically (if deployed).
    // but we will manually update it here as fallback until we are sure DB triggers are synced.
