...
import { SKU, RESTOCK_PHONE } from "../config.ts";
...
async function handleNxWallet(phoneNumber: string, lang: string, parts: string[], user: any) {
  ...
  if (action === "1") {
    ...
    const { data: stats } = await supabase.from("v_merchant_stats_daily")
      .select("*")
...
async function handleSettings(phoneNumber: string, lang: string, parts: string[], user: any) {
  ...
  if (choice === "2") {
      ...
      if (isNaN(pct) || pct < 10 || pct > 40) return `END ${t(lang, "invalid_pct")}`; // 10-30 or 40 depending on tier?
...
async function handleConfirmDelivery(phoneNumber: string, lang: string, parts: string[], user: any) {
  // Check for pending restock invoices or requests
  const { data: request } = await supabase.from("restock_requests")
    .select("*")
    .eq("merchant_code", user.merchant_code)
    .in("status", ["pending", "dispatched", "approved"])
    .order("requested_at", { ascending: false })
    .limit(1)
    .maybeSingle();

  if (!request) {
    return t(lang, "no_pending_delivery");
  }

  if (parts.length === 2) {
    return t(lang, "confirm_delivery_prompt");
  }
